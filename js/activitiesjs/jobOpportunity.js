...
document.getElementById("career-branch-1").addEventListener("click", function() { //if the player clicks on the first button (career-branch-1)//
	if (Job === "int-designer-select" && hasJob === false && Degree.includes("highSchool") && Age >= 18 && CriminalRecord.length === 0) { //and they meet the qualifications//
	...
	} else if (Job === "int-designer-select" && (hasJob === true || !Degree.includes("highSchool") || Age < 18 || CriminalRecord.length != 0)) { //but if they don't meet the qualifications//
	...
	if (Job === "engineer-select" && hasJob === false && Degree.includes("bachelors-engineering") && Age >= 18 && CriminalRecord.length === 0) {
		revent = "job-appr-engineer-accepted"; //they get the job//
		console.log("successfully applied for job as apprentice engineer");
		localStorage.setItem("revent", revent);
		window.location.href = "./event.html"; //redirects player to the events page//
	} else if (Job === "engineer-select" && (hasJob === true || !Degree.includes("bachelors-engineering") || Age < 18 || CriminalRecord.length != 0)) {
		revent = "job-appr-engineer-rejected"; //they don't get the job//
		console.log("did not successfully apply for job. reason: unqualified");
		localStorage.setItem("revent", revent);
		window.location.href = "./event.html";
	}
	if (Job === "ffw-select" && hasJob === false && Age >= 16 && Charisma >= 60) {
		revent = "job-ffw-cashier-accepted"; //they get the job//
		console.log("successfully applied for job as cashier");
		localStorage.setItem("revent", revent);
		window.location.href = "./event.html"; //redirects player to the events page//
	} else if (Job === "ffw-select" && (hasJob === true || Age < 16 || Charisma < 60)) {
		revent = "job-ffw-cashier-rejected"; //they don't get the job//
		console.log("did not successfully apply for job. reason: unqualified");
		localStorage.setItem("revent", revent);
		window.location.href = "./event.html";
	}
	if (Job === "scientist-select" && hasJob === false && Age >= 21 && SkillChemistry >= 60 && Degree.includes("bachelors-chemistry")) {
		revent = "job-chemistry-accepted"; //they get the job//
		console.log("successfully applied for job as chemist");
		localStorage.setItem("revent", revent);
		window.location.href = "./event.html"; //redirects player to the events page//
	} else if (Job === "scientist-select" && (hasJob === true || Age < 21 || SkillChemistry < 60 || !Degree.includes("bachelors-chemistry"))) {
...
document.getElementById("career-branch-2").addEventListener("click", function() {
	if (Job === "int-designer-select" && hasJob === false && Degree.includes("highSchool") && Age >= 20 && CriminalRecord.length === 0) {
		revent = "job-int-designer-accepted";
		console.log("successfully applied for job as standard interior designer");
		localStorage.setItem("revent", revent);
		window.location.href = "./event.html";
	} else if (Job === "int-designer-select" && (hasJob === false || !Degree.includes("highSchool") || Age < 20 || CriminalRecord.length != 0)) {
		revent = "job-int-designer-rejected";
		console.log("did not successfully apply for job. reason: unqualified");
		localStorage.setItem("revent", revent);
		window.location.href = "./event.html";
	}
	if (Job === "engineer-select" && hasJob === false && Degree.includes("bachelors-engineering") && Degree.includes("bachelors-mathematics") && Age >= 24 && workExpCalculator("appr-engineer") >= 3 && CriminalRecord.length === 0) {
		revent = "job-engineer-accepted"; //they get the job//
		console.log("successfully applied for job as apprentice engineer");
		localStorage.setItem("revent", revent);
		window.location.href = "./event.html"; //redirects player to the events page//
	} else if (Job === "engineer-select" && (hasJob === true || !Degree.includes("bachelors-engineering") || !Degree.includes("bachelors-mathematics") || Age < 24 || workExpCalculator("appr-engineer") < 3 || CriminalRecord.length != 0)) {
		revent = "job-engineer-rejected"; //they don't get the job//
		console.log("did not successfully apply for job. reason: unqualified");
		localStorage.setItem("revent", revent);
		window.location.href = "./event.html";
	}
	if (Job === "ffw-select" && hasJob === false && Age >= 16 && SkillCooking >= 60) {
		revent = "job-ffw-cook-accepted"; //they get the job//
		console.log("successfully applied for job as cook");
		localStorage.setItem("revent", revent);
		window.location.href = "./event.html"; //redirects player to the events page//
	} else if (Job === "ffw-select" && (hasJob === true ||  Age < 16 || SkillCooking < 60)) {
		revent = "job-ffw-cook-rejected"; //they don't get the job//
		console.log("did not successfully apply for job. reason: unqualified");
		localStorage.setItem("revent", revent);
		window.location.href = "./event.html";
	}
	if (Job === "scientist-select" && hasJob === false && Age >= 21 && SkillPhysics >= 60 && Degree.includes("bachelors-physics")) {
		revent = "job-physicist-accepted"; //they get the job//
		console.log("successfully applied for job as physicist");
		localStorage.setItem("revent", revent);
		window.location.href = "./event.html"; //redirects player to the events page//
	} else if (Job === "scientist-select" && (hasJob === true ||  Age < 21 || SkillPhysics < 60 || !Degree.includes("bachelors-physics"))) {
...
document.getElementById("career-branch-3").addEventListener("click", function() {
	if (Job === "int-designer-select" && hasJob === false && Degree.includes("masters-int-design") && workExpCalculator("int-designer") >= 4 && CriminalRecord.length === 0) {
		revent = "job-sr-int-designer-accepted";
		console.log("successfully applied for job as standard interior designer");
		localStorage.setItem("revent", revent);
		window.location.href = "./event.html";
	} else if (Job === "int-designer-select" && (hasJob === false || !Degree.includes("masters-int-design") || workExpCalculator("int-designer") < 4 || CriminalRecord.length != 0)) {
		revent = "job-sr-int-designer-rejected";
		console.log("did not successfully apply for job. reason: unqualified");
		localStorage.setItem("revent", revent);
		window.location.href = "./event.html";
	}
	if (Job === "engineer-select" && hasJob === false && Degree.includes("masters-engineering") && Degree.includes("masters-mathematics") && workExpCalculator("engineer") >= 5 && Age >= 26 && CriminalRecord.length === 0) {
		revent = "job-sr-engineer-accepted"; //they get the job//
		console.log("successfully applied for job as apprentice engineer");
		localStorage.setItem("revent", revent);
		window.location.href = "./event.html"; //redirects player to the events page//
	} else if (Job === "engineer-select" && (hasJob === true || !Degree.includes("masters-engineering") || !Degree.includes("masters-mathematics")|| workExpCalculator("engineer") < 5 || Age < 26 || CriminalRecord.length != 0)) {
		revent = "job-sr-engineer-rejected"; //they don't get the job//
		console.log("did not successfully apply for job. reason: unqualified");
		localStorage.setItem("revent", revent);
		window.location.href = "./event.html";
	}
	if (Job === "ffw-select" && hasJob === false && Age >= 21 && SkillManagement >= 70) {
		revent = "job-ffw-manager-accepted"; //they get the job//
		console.log("successfully applied for job as branch manager");
		localStorage.setItem("revent", revent);
		window.location.href = "./event.html"; //redirects player to the events page//
	} else if (Job === "engineer-select" && (hasJob === true || Age < 21 || SkillManagement < 70)) {
		revent = "job-ffw-manager-rejected"; //they don't get the job//
		console.log("did not successfully apply for job. reason: unqualified");
		localStorage.setItem("revent", revent);
		window.location.href = "./event.html";
	}
	if (Job === "scientist-select" && hasJob === false && Age >= 21 && SkillBiology >= 60 && Degree.includes("bachelors-biology")) {
		revent = "job-biologist-accepted"; //they get the job//
		console.log("successfully applied for job as biologist");
		localStorage.setItem("revent", revent);
		window.location.href = "./event.html"; //redirects player to the events page//
	} else if (Job === "scientist-select" && (hasJob === true || Age < 21 || SkillBiology < 60 || !Degree.includes("bachelors-biology"))) {
...
document.getElementById("career-branch-4").addEventListener("click", function() {
	if (Job === "scientist-select" && hasJob === false && Age >= 21 && SkillChemistry >= 80 && SkillEngineering >= 60 && workExpCalculator("chemist") >= 5 && Degree.includes("bachelors-mathematics") && Degree.includes("masters-chemistry")) {
		revent = "job-chemical-engineer-accepted"; //they get the job//
		console.log("successfully applied for job as chemical engineer");
		localStorage.setItem("revent", revent);
		window.location.href = "./event.html"; //redirects player to the events page//
	} else if (Job === "scientist-select" && (hasJob === true || Age < 21 || SkillChemisty < 80 || SkillEngineering < 60 || workExpCalculator("chemist") < 5 || !Degree.includes("bachelors-mathematics") || !Degree.includes("masters-chemistry"))) {
...
document.getElementById("career-branch-5").addEventListener("click", function() {
	if (Job === "scientist-select" && hasJob === false && Age >= 21 && SkillPhysics >= 90 && workExpCalculator("physicist") >= 5 && Degree.includes("masters-physics") && Degree.includes("bachelors-engineering")) {
		revent = "job-particle-physicist-accepted"; //they get the job//
		console.log("successfully applied for job as biologist");
		localStorage.setItem("revent", revent);
		window.location.href = "./event.html"; //redirects player to the events page//
	} else if (Job === "scientist-select" && (hasJob === true || Age < 21 || SkillPhysics < 90 || workExpCalculator("physicist") < 5 || !Degree.includes("masters-physics") || !Degree.includes("bachelors-engineering"))) {
...
document.getElementById("career-branch-6").addEventListener("click", function() {
	if (Job === "scientist-select" && hasJob === false && Age >= 21 && SkillBiology >= 80 && SkillChemistry >= 80 && workExpCalculator("biologist") >= 4 && workExpCalculator("chemist") >= 4 && Degree.includes("masters-biology") && Degree.includes("masters-chemistry")) {
		revent = "job-biochemist-accepted"; //they get the job//
		console.log("successfully applied for job as biochemist");
		localStorage.setItem("revent", revent);
		window.location.href = "./event.html"; //redirects player to the events page//
	} else if (Job === "scientist-select" && (hasJob === true || Age < 21 || SkillBiology < 80 || SkillChemistry < 80 || workExpCalculator("biologist") < 4 || workExpCalculator("chemist") < 4 || !Degree.includes("masters-biology") || !Degree.includes("masters-chemistry"))) {
